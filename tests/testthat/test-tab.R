...
test_that("tab function prints correct tables for mtcars", {
  # Capture and snapshot the output
  expect_snapshot_output(tab(df, c("mpg", "cyl", "disp")))
})
...
test_that("tab gives error if vars argument is not a character vector", {

  expect_error(tab(df, 999), "The 'vars' argument must be a character vector (see docs for examples).", fixed = TRUE)
})
